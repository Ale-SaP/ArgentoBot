...
class Example(commands.Cog):
    ...
    async def on_ready(self):
        print(f"Morse cog has been loaded!")

    @commands.command (aliases=["morsehelp"])
    async def MORSEHELP(self, ctx):
        await ctx.send(ArgentoTxt.MorseHelp)
    
    @commands.command (aliases=["Morse", "MORSE", "morse"])
    async def translationToMorse(self, ctx ,*args):
